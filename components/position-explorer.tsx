...
export function PositionExplorer({points,baseline,day,config,language,preset,onScenario,onDay}:{points:Point[];baseline:Point[];day:number;config:Config;language:Language;preset:string;onScenario:(id:string)=>void;onDay:(day:number)=>void}){
 ...
 return <section className="position-explorer" aria-labelledby="position-title">
  ...
  <div className="position-layout"><div className="position-inputs">
   ...
   <div className="control"><div className="control-label"><span>{t('Branches in one NFT')}</span><output>{branches} / 10</output></div><Slider aria-label={t('Branches in one NFT')} value={[branches]} min={1} max={10} step={1} onValueChange={n=>{const next=Array.isArray(n)?n[0]:n;setBranches(next);setRetire(r=>Math.min(r,next));}}/><p>{t('One NFT starts with one branch. Extra branches require expansion; their cost is not modeled here.')}</p></div>
